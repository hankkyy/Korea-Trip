...
- Service Worker 当前版本：`lu-travel-v73`；CloudBase Web SDK 随站点发布于 `assets/vendor/cloudbase.full.js`，登录不再依赖外部 CDN。页面启动不等待 IndexedDB，iPhone/PWA 的 IndexedDB 操作 1.5 秒超时后降级为 localStorage 与内存缓存，避免动态数据一直停在加载状态。灵感箱采用手动单按钮新建，不读取剪贴板或上传图片；卡片按稳定记录 ID 分配薄荷、蜜桃、淡紫、天空蓝、奶油黄或玫瑰粉色。跨设备轮询在启动、回到前台和每 4 秒执行。CloudBase 根入口和 `/korea/` 镜像分别缓存自己的页面壳，避免旧根缓存覆盖镜像。
