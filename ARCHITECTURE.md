...
更新时间：2026-09-11
对应代码版本：`6a5e476`；前端缓存版本：`lu-travel-v77`
...
| 私有文件 | CloudBase Storage `private/…` | 内置凭证和未来共同附件；当前规则为 PRIVATE |
...
当前限制：协议仍按列表快照提交，没有页面内冲突选择器、逐字段同步、通用旅程成员模型或版本浏览/回滚界面。它降低了已知的重复、闪空和乱序风险，但不能替代真实两人双设备弱网验证。
...
目前浏览器将手动附件读取为 data URL 后随 `/docs` 同步快照保存。非图片附件限制为 1.5 MB，以避免超过 HTTP API 6 MB 请求体上限。这会带来体积、重试和共享权限边界问题，不能视为长期方案。

### 必须完成的目标实现

1. 浏览器上传到私有存储，并只把文件 ID、名称、MIME、大小和必要元数据写入同步记录。
2. 服务端先校验共同所有者和 `tripId` 权限，再使用服务端身份签发短期下载/预览 URL。
3. 所有者可访问共同文件；访客无法枚举文件、无法得到临时 URL。
4. 上传失败不修改现有文件引用；成功后再原子更新记录；保留旧文件供回滚或明确清理。
...
已自动覆盖：静态检查、同步单测、浏览器多 Tab/离线队列/动态数据族回归、访客边界、灵感增改和历史私有文件 URL 迁移、跨尺寸 UI 审查。
