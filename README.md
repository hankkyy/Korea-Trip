...
- 支出、随笔和文件默认共同可见；普通随笔直接保存，需要时可单独设置密码锁。
...
- 离线：Service Worker `lu-travel-v86`，配合 localStorage、IndexedDB 持久队列和前台/4 秒轮询同步。
- 发布：Vercel 生产入口 `https://www.jinlu.cloud/`，以及 CloudBase 根入口和 `/korea/` 镜像。两个 CloudBase 路径必须各自注册作用域正确的 Service Worker，避免页面壳互相污染。

## 数据可靠性原则

数据一致性与持久化高于界面和新功能。每次修改先持久化进本机写入队列，再由服务端做版本检查、幂等处理、不可变历史和三方合并。不同记录的并发修改自动合并；同一记录同时修改时选择 `updatedAt` 最新的版本。旧队列或版本冲突会自动拉取远端快照、保留远端新增记录并重试，不要求用户导入、导出或手动处理同步。页面只显示“正在保存”或“已保存”，关闭页面时仍会保护尚未落云的修改。
...
用户选择的附件会先上传到 CloudBase PRIVATE 存储；同步记录只保存 `cloud://` 文件 ID、名称和类型等元数据。预览和下载由 `korea-api` 校验 `tripId` 与文件路径后签发 15 分钟临时地址。照片会自动缩小到适合上传的大小；上传会自动重试，失败时不会替换原附件。PDF 先读取为站内 Blob 地址再交给预览器，避免签名直链触发自动下载。
