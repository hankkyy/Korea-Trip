...
- 釜山 / 首尔双城市关系图（纯前端水彩示意图，无第三方地图 SDK 主依赖；Apple Maps 为主跳转入口，Naver / Kakao / Google / Amap + 坐标作为备用）
- 出发 / 跨年 / 回程 三枚倒计时圆环
...
- PWA：Add to Home Screen（standalone 模式）+ Service Worker 离线壳（缓存版本 `korea-trip-v13`）
